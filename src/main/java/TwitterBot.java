...
public class TwitterBot {
    ...
    while(true) {
      for (String aSUBREDDITS_TO_MONITOR : SUBREDDITS_TO_MONITOR) {
        ...
        for (int i = 0; i < LIMIT_OF_SUBMISSIONS; i++) {
          ...
          // Wait two minutes before tweeting again
          Thread.sleep(120000);
        }
      }
  ...
  private static String getImage(String link) {
    ...
    String extension;
    try {
      // Images from https://i.reddituploads.com/ are stupid and the '&'s get replaced replaced with '&amp;' which is the HTML entity for the '&' symbol.
      ...
      // This is so ugly, there has to be a better way to do this :/
      ...
      // Credit to http://www.technicalkeeda.com/java-tutorials/ for bit that downloads image to directory
